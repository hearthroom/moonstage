/**
 * uni.showModal 的 HarperHarbor 預設按鈕色。
 *
 ...
 * cancelColor #000（暗底上根本看不見），兩個都不適合 HarperHarbor 的暗色介面。
 *
 * 樣式面（面板 / 遮罩 / 圓角）在 common/uni-modal.css，僅 H5 生效；
 * APP-PLUS 走系統原生彈窗，一致性只靠這裡的顏色參數。
 */
export const STAGE_CONFIRM_COLOR = '#F5C542'
export const STAGE_CANCEL_COLOR = 'rgba(232, 234, 237, 0.72)'

type ModalOptions = Record<string, unknown>

export function withModalDefaults<T extends ModalOptions>(options: T): T {
  return {
    confirmColor: STAGE_CONFIRM_COLOR,
    cancelColor: STAGE_CANCEL_COLOR,
    ...(options || {}),
  } as T
}
