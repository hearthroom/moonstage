...
export interface SandboxHostDeps {
  ...
  /** 殼裡頁首的實際底色變了（給系統狀態列用）。 */
  onChromeColor?(color: string | null, bottom?: string | null): void
  /**
  ...
  const onMessage = (event: MessageEvent) => {
    ...
    switch (message.type) {
      ...
      case 'docstate':
        ...
        return
      case 'chrome-color':
        if (deps.onChromeColor) deps.onChromeColor(
          typeof message.color === 'string' ? message.color : null,
          typeof message.bottom === 'string' ? message.bottom : message.bottom === null ? null : undefined,
        )
        return
      case 'panel.ui':
