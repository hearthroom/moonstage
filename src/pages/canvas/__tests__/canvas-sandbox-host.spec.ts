...
describe('視窗高度與連結', () => {
  ...
  // 鍵盤開著時視覺視窗變矮：送可見的那一段給殼，殼的根跟著縮，輸入區才在鍵盤上方
  // （owner 2026-09-22 iOS 面板數值：可見 447、殼的根仍 796、輸入框被蓋住）。
  it('非全螢幕：visualViewport 變矮就送可見高度，回復就送整高', async () => {
    ...
    try {
      ...
      await settle()
      expect(h.posted.filter((m) => m.type === 'viewport')).toEqual([{ ms: 1, type: 'viewport', height: 447 }])
      vv.height = window.innerHeight; vv.dispatchEvent(new Event('resize'))
      await settle()
