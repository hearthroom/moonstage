...
describe('聊天串流位址', () => {
  it('嵌入宿主的 API 主機同時決定串流主機，優先於建置與開發設定', () => {
    expect(resolveChatWebSocketBase('wss://api.harperharbor.com', 'localhost:8888', 'https://api.harperharbor.com/'))
      .toBe('wss://api.harperharbor.com/open/v1/conversation/ws')
    expect(resolveChatWebSocketBase('wss://api.harperharbor.com', undefined, 'http://127.0.0.1:8915'))
      .toBe('ws://127.0.0.1:8915/open/v1/conversation/ws')
  })
  it('走開放 API v1 的串流路徑', () => {
    expect(resolveChatWebSocketBase('wss://api.harperharbor.com'))
      .toBe('wss://api.harperharbor.com/open/v1/conversation/ws')
  })
...
describe('指向臨時實例', () => {
  it('明確給了位址就用它，不再落回設定值', () => {
    expect(resolveChatWebSocketBase('wss://api.harperharbor.com', '203.0.113.10:9000'))
  ...
  it('位址帶了協定也接受——手貼的時候很容易連 http:// 一起貼進來', () => {
    expect(resolveChatWebSocketBase('wss://api.harperharbor.com', 'http://203.0.113.10:9000'))
      .toBe('ws://203.0.113.10:9000/open/v1/conversation/ws')
  })
})
