 ...
 * 氣泡在 `.scroll-view`（overflow:auto 的捲動容器）底下。Android Chrome 上它照規範算到
 * 視窗、蓋住整頁；iPhone 上同一張卡的「滿版舞台」只鋪滿訊息區，頁首與輸入區蓋在它上面
 * （2026-09-24 社群回報截圖）。最合理的解釋是 iOS Safari 把捲動容器裡的 fixed 後代裁在
 * 容器框內——尚未有真機數值證實，但不論是裁切還是層級被困，搬進作者容器都脫離了捲動
 * 容器，兩端都照作者選的層級畫。
 *
